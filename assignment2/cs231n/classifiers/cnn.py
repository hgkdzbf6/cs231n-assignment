...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        H_ = filter_size
        W_ = filter_size
        self.params['W1'] = np.random.randn(F, channel, H_, W_) * weight_scale
        self.params['b1'] = np.zeros((F))
        self.params['W2'] = np.random.randn(H*W//4*F, hidden_dim )
        self.params['b2'] = np.zeros(hidden_dim)
        self.params['W3'] = np.random.rand(hidden_dim,C  )
    ...
    def loss(self, X, y=None):
        ...
        loss +=self.reg * 0.5 * (np.sum(W1**2) + np.sum(W2**2) + np.sum(W3**3))

        dout3, dW3, db3 = affine_backward(dout,cache3)
        dout2, dW2, db2 = affine_relu_backward(dout3, cache2)
        dout, dW1, db1 = conv_relu_pool_backward(dout2,cache)        
